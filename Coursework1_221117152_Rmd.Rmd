...
The search volume returns to pre-covid levels in 2022 after the loosening of covid restrictions in GB. There is a slight increase in search history in 2025.
```{r echo=FALSE}
plot(exercise_decom$trend, main = "Trend from the decomposed time series")
```

### 2.2 Seasonality
Seasonality component shows a clear decreasing trend in search history for exercise over the year. It is interesting to see a high search history in January, and steep reductions between April to August and between November to December. An intuitive explanation for the high volume in January is most people try to loose weight after the holiday season and make new year resolutions during this month. Gradually over the year, people pay less attention on exercising hence search volume reduces.
```{r echo=FALSE}
...
First, we model the complete time series without subsetting it.
```{r echo=FALSE}
library(prophet)
library(zoo)
library(ggplot2)
...
ARIMA is another widely used time series forecasting tool in R. It combines auto-regression, differencing, and moving average techniques to analyse a time series. Apply an alternative method of modelling to the post-covid data using ARIMA.
```{r}
library(forecast)
exercise_subset <- window(gb_exercise, start = c(2022,1))
...
To test the modelled time series (from the Prophet and ARIMA method), we can compare the forecasts with actual data from January to March 2026. The following is the indexed web search history for "exercise" in GB in 2026.
